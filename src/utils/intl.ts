...
export function formatRelativeTime(
  ...
  const { language, relativeTo = Temporal.Now.zonedDateTimeISO('UTC'), ...formatOptions } = options
...
export function formatDuration(
  ...
  if (Intl.DurationFormat) {
    ...
    if (padZero) {
      ...
      return parts
        .map((part) => {
          if (part.type === 'integer' && (part.unit === 'minute' || part.unit === 'second')) {
 ...
 */
/*#__NO_SIDE_EFFECTS__*/
export function formatLanguageName<T extends Intl.DisplayNamesFallback = 'code'>(
  language: string,
